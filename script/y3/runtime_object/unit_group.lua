...
---@return integer
--单位组中某个状态的单位数量    --转ECA出错
function unit_group:get_number_of_units_in_specified_state_in_unit_group()
    return
end
...
---筛选范围内的所有单位
function unit_group.filter_unit_in_area(point, shape, owner_player, visible_player, invisible_player, not_in_unit_group, has_tag,
                        not_has_tag, unit_name, not_unit, unit_type, in_state, not_in_state, is_including_dead, max_count, filtering_rules)

    local py_unit_group = game_api.filter_unit_id_list_in_area_v2(point.base(), shape, owner_player and owner_player.base() or nil, visible_player and visible_player.base() or nil, 
    invisible_player and invisible_player.base() or nil, not_in_unit_group and not_in_unit_group.base() or nil, has_tag, not_has_tag,unit_name, not_unit, unit_type, in_state, 
